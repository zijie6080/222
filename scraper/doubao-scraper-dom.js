/**
 * 豆包 Doubao 对话抓取脚本（DOM 版）
 * ------------------------------------------------------------------
 * 用法：
 *   1. 登录 https://www.doubao.com，确保左侧历史对话侧边栏展开
 *   2. F12 打开开发者工具 → Console
 *   3. 粘贴整个脚本运行，脚本会自动：加载全部历史对话 → 逐个点开 →
 *      滚动加载完整历史 → 抓取 → 下载 conversations.json
 *
 * 说明：豆包 Web 端无公开 REST 接口，故用 DOM 方式抓取。局限：
...
(async () => {
  ...
  const log = (...a) => { console.log('%c[doubao-export]', 'color:#0b5fff;font-weight:bold', ...a); report('info', a.map(String).join(' ')); };
  const warn = (...a) => { console.warn('[doubao-export]', ...a); report('warn', a.map(String).join(' ')); };
  ...
  function itemTitle(el) {
    ...
    const t = el.querySelector('[class*="title"], [class*="name"]');
    return ((t ? t.textContent : el.textContent) || '').trim().slice(0, 120) || '(无标题)';
  }

  // ---- 1. 滚动侧边栏加载全部对话 ----
  async function loadAllSidebarItems() {
    let first = document.querySelector(convItemSelector);
    if (!first) throw new Error('侧边栏里找不到对话，请确认侧边栏已展开、已登录 doubao.com');
  ...
  function isUserMessage(el) {
    ...
    if (/send/.test(tid)) return true;
    if (/receive/.test(tid)) return false;
    const cls = el.className || '';
    return /send|user|right/i.test(String(cls)) && !/receive|answer/i.test(String(cls));
  }
  ...
  for (let i = 0; i < total; i++) {
    ...
    try {
      ...
      conversations.push({
        id: (location.pathname.match(/\/chat\/([\w-]+)/) || [])[1] || `doubao-${i + 1}`,
        title,
        createTime: null,
        updateTime: null,
        messages,
