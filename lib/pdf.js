  ...
  try {
    ...
    if (opts.merge) {
      ...
      for (let iter = 0; iter < 5; iter++) {
        ...
        const buf = await renderPdfBuffer(page, buildTocOnlyHtml(conversations, starts));
      ...
      // ---- 第 2 遍：带真实页码整体渲染 ----
      const starts = computeStartPages(tocPages, pageCounts);
      const outPath = path.join(opts.output, 'chatgpt-export.pdf');
      await page.setContent(buildMergedHtml(conversations, { pageNumbers: starts }), { waitUntil: 'load' });
      ...
      await page.setContent(buildIndexHtml(conversations, files, '点击标题打开对应 PDF 文件'), { waitUntil: 'load' });
