...
export const DEFAULT_DOC_TITLE = '聊天记录导出';

/**
 * 目录。pageNumbers 为 null 时（HTML 格式）不显示页码列。
 * hrefs 缺省时使用文内锚点 #conv-i；分文件模式下传入各文件相对路径。
 */
export function renderToc(conversations, { pageNumbers = null, hrefs = null, subtitle = '', docTitle = DEFAULT_DOC_TITLE } = {}) {
  const rows = conversations
    ...
    .join('\n');
  return `<h1 class="doc-title">${escapeHtml(docTitle)}</h1>
...
export function buildMergedHtml(conversations, { pageNumbers = null, docTitle = DEFAULT_DOC_TITLE } = {}) {
  const toc = renderToc(conversations, { pageNumbers, docTitle });
  const sections = conversations
    .map((c, i) => renderConversationSection(c, i, conversations.length))
    .join('\n');
  return buildDocument({ title: docTitle, bodyHtml: `${toc}\n${sections}` });
}
...
export function buildTocOnlyHtml(conversations, pageNumbers, docTitle = DEFAULT_DOC_TITLE) {
  return buildDocument({
    title: '目录',
    bodyHtml: renderToc(conversations, { pageNumbers, docTitle }),
  });
}

/** 分文件模式下的索引页（链接指向各文件） */
export function buildIndexHtml(conversations, hrefs, subtitle, docTitle = DEFAULT_DOC_TITLE) {
  return buildDocument({
    title: `${docTitle} - 索引`,
    bodyHtml: renderToc(conversations, { hrefs, subtitle, docTitle }),
  });
}
