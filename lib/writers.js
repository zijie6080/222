...
function txtToc(conversations, docTitle) {
...
export async function exportTxt(conversations, opts, log) {
  const written = [];
  if (opts.merge) {
    const outPath = path.join(opts.output, `${opts.baseName}.txt`);
    const body = conversations.map((c, i) => conversationToTxt(c, i, conversations.length)).join('\n\n');
    await fs.writeFile(outPath, `${txtToc(conversations, opts.docTitle)}\n\n${body}`, 'utf8');
    ...
    await fs.writeFile(indexPath, txtToc(conversations, opts.docTitle), 'utf8');
...
export async function exportMd(conversations, opts, log) {
  ...
  if (opts.merge) {
    ...
    const body = conversations
      ...
      .join('\n\n---\n\n');
    const outPath = path.join(opts.output, `${opts.baseName}.md`);
...
export async function exportHtml(conversations, opts, log) {
  const written = [];
  if (opts.merge) {
    const outPath = path.join(opts.output, `${opts.baseName}.html`);
    await fs.writeFile(outPath, buildMergedHtml(conversations, { docTitle: opts.docTitle }), 'utf8');
    written.push(outPath);
  } else {
    const files = [];
    for (let i = 0; i < conversations.length; i++) {
    ...
    await fs.writeFile(indexPath, buildIndexHtml(conversations, files, '点击标题打开对应对话', opts.docTitle), 'utf8');
    written.push(indexPath);
  }
