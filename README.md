...
4. 点工具栏里的插件图标，弹窗会自动识别当前平台，按需调整设置
   （建议第一次把「最多抓取对话数」填 5 试跑）；
5. 点**「开始抓取并导出」**，弹窗里实时显示进度，完成后自动下载所选格式的
   文件。关闭弹窗不会中断抓取。
...
| 分组 | 设置项 |
| --- | --- |
| 抓取范围 | 最多抓取对话数；起始/截止日期；标题关键词；**选择对话**（先拉取清单、逐条勾选要导出的对话，支持搜索和全选）；包含已归档对话（仅 ChatGPT） |
| 导出内容 | 消息时间戳；思考过程（reasoning）；系统消息/工具输出；原对话链接 |
| 导出格式 | JSON / Markdown / TXT / HTML 多选；合并为一个文件或每个对话单独文件 |
| 文件设置 | 文件名前缀（默认「平台名-」）；时间格式（24h / 中文日期 / 12h） |
