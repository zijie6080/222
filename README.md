...
> **DeepSeek / Kimi / 智谱 / 腾讯元宝：接口拿不到清单会自动退回「页面抓取」**。这些平台接口
> 未公开、我无法逐一验证，若接口调用返回空或失败，脚本会自动改从**左侧对话侧边栏**直接识别对话
> （排除「新建会话/插件/设置」等导航项，优先可滚动的对话列表容器），逐个点开抓取正文；此路
> 不含时间戳、代码围栏，角色按左右位置判定。控制台会打印「改用页面抓取（DOM 兜底）」。若两条路
> 都识别不到，会提示你把接口响应发回以精确修复。
